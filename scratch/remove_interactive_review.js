...
// Remove buttons
const buttonsStart = htmlContent.indexOf(
  '<div class="mt-auto pt-5 flex gap-3 border-t border-[var(--draya-neutral-100)]">',
);
const buttonsEnd =
  htmlContent.indexOf(
    '</div>',
    htmlContent.indexOf('</button>', htmlContent.indexOf('</button>', buttonsStart) + 9) + 9,
  ) + 6;
...
tsContent = tsContent.replace(
  /readonly showReviewModal = signal<boolean>\(false\);\s*readonly reviewModalData = signal<\{topic: string; data: InteractiveReviewDto \| null\}>\(\{topic: '', data: null\}\);\s*readonly isLoadingReview = signal<boolean>\(false\);\s*readonly reviewError = signal<string \| null>\(null\);/g,
  '',
);
...
// Remove methods
const methodsRegex =
  /openInteractiveReview\(topicName: string\): void \{[\s\S]*?generatePracticeExam\(topicName: string\): void \{[\s\S]*?\}\s*\}\s*approveReport\(\)/g;
