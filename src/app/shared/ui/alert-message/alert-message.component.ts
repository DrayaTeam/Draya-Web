...
@Component({
  ...
  template: `
    <div [class]="'flex items-start gap-3 rounded-md p-4 text-sm ' + typeClasses[type]">
      <span [class]="iconClasses[type] + ' mt-0.5 h-5 w-5 shrink-0'"></span>
      <div>
        @if (title) {
...
export class AlertMessageComponent {
  ...
  protected typeClasses = {
    ...
    success:
      'bg-green-50 text-green-900 border border-green-200 dark:bg-green-950/50 dark:text-green-200 dark:border-green-900',
    warning:
      'bg-yellow-50 text-yellow-900 border border-yellow-200 dark:bg-yellow-950/50 dark:text-yellow-200 dark:border-yellow-900',
    error:
