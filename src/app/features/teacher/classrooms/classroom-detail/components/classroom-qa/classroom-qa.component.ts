import { Component, ChangeDetectionStrategy, inject, signal, effect, input, computed, HostListener } from '@angular/core';
...
import { QuestionDto, QuestionFilters, QuestionReplyDto } from '../../../../../../core/models/qa.model';
...
export class ClassroomQaComponent {
  ...
  loadQuestions(id: string, filters: QuestionFilters): void {
    ...
    this.qaService.getQuestions(id, filters)
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.messageService?.add({ severity: 'error', summary: 'خطأ', detail: 'فشل تحميل الأسئلة. يرجى المحاولة لاحقاً.' });
        }
      });
  }

  updateFilterBy(filterValue: string): void {
    this.filters.update(f => ({ ...f, filterBy: filterValue as QuestionFilters['filterBy'], pageNumber: 1 }));
  }

  updateSortBy(sortValue: string): void {
    this.filters.update(f => ({ ...f, sortBy: sortValue as QuestionFilters['sortBy'], pageNumber: 1 }));
  }
  ...
  private loadReplies(questionId: string): void {
    ...
    this.qaService.getQuestionDetails(this.classroomId(), questionId)
      .pipe(finalize(() => {
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.messageService?.add({ severity: 'error', summary: 'خطأ', detail: 'فشل تحميل الردود.' });
        }
      });
  }
    ...
    request$
      ...
      .subscribe({
        next: (newReply) => {
          ...
          this.messageService?.add({ severity: 'success', summary: 'نجاح', detail: 'تم إضافة الرد بنجاح.' });
        },
        error: (err) => {
          console.error('Failed to post reply', err);
          this.messageService?.add({ severity: 'error', summary: 'خطأ', detail: 'فشل إضافة الرد.' });
        }
      });
  }
  ...
  onImageSelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (file.type.startsWith('image/')) {
        ...
        this.messageService?.add({ severity: 'warn', summary: 'صيغة غير مدعومة', detail: 'الرجاء اختيار صورة صالحة.' });
      }
    ...
    request$
      ...
      .subscribe({
        next: (newQ) => {
          ...
          this.messageService?.add({ severity: 'success', summary: 'نجاح', detail: 'تم نشر السؤال بنجاح.' });
        },
        error: (err: ApiError) => {
  ...
  saveEditQuestion(questionId: string): void {
    ...
    this.qaService.updateQuestion(this.classroomId(), questionId, this.editQuestionContent).subscribe({
      next: () => {
        this.questions.update(qs => qs.map(q => 
          q.id === questionId ? { ...q, content: this.editQuestionContent } : q
        ));
        this.cancelEditQuestion();
        this.messageService?.add({ severity: 'success', summary: 'نجاح', detail: 'تم تعديل السؤال بنجاح.' });
      },
      error: (err) => {
        console.error('Failed to update question', err);
        this.messageService?.add({ severity: 'error', summary: 'خطأ', detail: 'حدث خطأ أثناء تعديل السؤال.' });
      }
    });
  }
  ...
  deleteQuestion(questionId: string): void {
    ...
    this.qaService.deleteQuestion(this.classroomId(), questionId).subscribe({
      next: () => {
        ...
        this.messageService?.add({ severity: 'success', summary: 'نجاح', detail: 'تم حذف السؤال بنجاح.' });
      },
      error: (err) => {
        console.error('Failed to delete question', err);
        this.messageService?.add({ severity: 'error', summary: 'خطأ', detail: 'حدث خطأ أثناء حذف السؤال.' });
      }
    });
  }
  ...
  toggleVote(question: QuestionDto, event?: Event): void {
    ...
    this.qaService.toggleVote(this.classroomId(), question.id, question.hasVoted)
      .pipe(finalize(() => {
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.messageService?.add({ severity: 'error', summary: 'خطأ', detail: 'فشل تسجيل التصويت.' });
        }
      });
  }
    ...
    this.qaService.deleteReply(this.classroomId(), questionId, replyId).subscribe({
      next: () => {
        this.threadReplies.update(map => {
          ...
          next.set(questionId, replies.filter(r => r.id !== replyId));
          return next;
        });
        ...
        this.messageService?.add({ severity: 'success', summary: 'نجاح', detail: 'تم حذف الرد بنجاح.' });
      },
      error: (err) => {
        console.error('Failed to delete reply', err);
        this.messageService?.add({ severity: 'error', summary: 'خطأ', detail: 'حدث خطأ أثناء حذف الرد.' });
      }
    });
  }
  ...
  saveEditReply(questionId: string, replyId: string): void {
    ...
    this.qaService.updateReply(this.classroomId(), questionId, replyId, this.editReplyContent).subscribe({
      next: () => {
        this.threadReplies.update(map => {
          const next = new Map(map);
          const replies = next.get(questionId) || [];
          next.set(questionId, replies.map(r => r.id === replyId ? { ...r, content: this.editReplyContent } : r));
          return next;
        });
        this.cancelEditReply();
        this.messageService?.add({ severity: 'success', summary: 'نجاح', detail: 'تم تعديل الرد بنجاح.' });
      },
      error: (err) => {
        console.error('Failed to update reply', err);
        this.messageService?.add({ severity: 'error', summary: 'خطأ', detail: 'حدث خطأ أثناء تعديل الرد.' });
      }
    });
  }
